...
import { entrar, cadastrar } from "./services/api.js";
...
const viewPorPerfil = (perfil) =>
  perfil === "gerente" ? "gerente" : perfil === "transportador" ? "entregador" : "loja";

const usuarioSalvo = () => carregarState("tccUsuario", null);

function App() {
  const [view, setView] = useState(() => {
    const salvo = usuarioSalvo();
    return salvo ? viewPorPerfil(salvo.perfil) : "login";
  });
  ...
  useEffect(() => {
    if (usuarioLogado) localStorage.setItem("tccUsuario", JSON.stringify(usuarioLogado));
  }, [usuarioLogado]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const resposta =
        modo === "login"
          ? await entrar(perfil, campos.email, campos.senha)
          : await cadastrar(perfil, campos);

      localStorage.setItem("tccToken", resposta.token);
      setUsuarioLogado({
  ...
  const sair = () => {
    ...
    localStorage.removeItem("tccUsuario");
...
function LoginScreen({ perfil, modo, campos, erro, carregando, handleChange, handleSubmit, mudarPerfil, setModo }) {
  return (
    <div style={styles.card}>
      ...
      <form onSubmit={handleSubmit} style={styles.form}>
        ...
        <button type="submit" style={{ ...styles.botao, ...(carregando ? { opacity: 0.7, cursor: "not-allowed" } : {}) }} disabled={carregando}>
          {carregando
            ? "Aguarde..."
            : modo === "login"
              ? "Entrar"
              : `Cadastrar ${PERFIS[perfil].label}`}
        </button>
      </form>
    </div>
  );
